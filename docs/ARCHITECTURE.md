...
> **Program vs data:** Crystallize (repo root) is one auditor program.
> Folders under `projects/<id>/` are interchangeable curriculum datasets.
> `./run-audit <id>` runs Layer 0 → Layer 1 → Layer 2 → hybrid synthesize (headline path).
> See [PIPELINE.md](PIPELINE.md), [DEPENDENCY_FLOW.md](../DEPENDENCY_FLOW.md), and [projects/STATUS.md](../projects/STATUS.md).

## Single responsibility

**Crystallize** = read-only **curriculum document auditor** (one program, any corpus).
...
│  Layer 0 elements → Layer 1 placement                   │
│  → Layer 2 structural completeness (code checklist)     │
└───────────────────────────┬─────────────────────────────┘
                            ▼
┌─────────────────────────────────────────────────────────┐
...
| **Models** | Judgment calls on messy filenames, Layer 0/1 semantics, hybrid first-pass / teacher narrative |
