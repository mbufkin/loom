...
3. **Graph + router:** HAS-PART belonging (`--with-graph`) feeds `route.py`.
   Filename is a prior (hard-wins for explicit lesson/quiz/exit/syllabus/pacing
   names). Graph Material roles and Assessment links assign Paths D/E/B when
   the filename is silent. Path C is leftover nursery, not “untyped TE.”
   Unknown/weak types are logged to `_loom_feedback.yaml` as data, not lost
4. **Path workflows (A–H):** run type-specific checks; **nothing is placed into
   a unit until it has been routed**
...
- **Generic fallback always works.** After filename + graph, leftovers route to
  Path C plus a feedback note, so a run never fails on an unrecognized document.
...
- [docs/PATHS.md](docs/PATHS.md) — graph-fed router cascade (content belonging)
