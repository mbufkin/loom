#!/usr/bin/env python3
"""workflows/student_practice.py — Path E (Student practice) E1–E5 presence.

Educational note: Path E reviews Learn / Practice / Succeed / worksheets —
student-facing practice materials, not TE facilitation (Path D) and not
quiz↔key pairs (Path B). Presence-first (E1–E4): scan Layer 0 excerpts with
the student_practice checklist; fall back to source text when the ledger is
empty so labs still smoke. Auditor-only — never invent items or keys.
E5 stays stubbed (one-pager). Lesson-objective alignment and LPS family
pairing stay out of presence depth.
"""
...
CHECKLIST_PATH = (
    Path(__file__).resolve().parent / "checklists" / "student_practice.yaml"
)


def load_student_practice_checklist() -> dict:
    """Load Path E checklist; empty dict if missing (tests can inject)."""
...
def e1_inventory(elements: list[dict], doc_id: str) -> dict:
    by_type = {t: len(v) for t, v in _by_type(elements).items()}
    return {
        "step": "E1",
        "status": "PRESENT" if elements else "MISSING",
        "note": "Layer 0 / source excerpt inventory for this student-practice doc",
...
def e_presence_for_step(
    elements: list[dict], checklist: dict, step: str
) -> dict:
    """Run one E2–E4 step: PRESENT/PARTIAL/MISSING/OPTIONAL_ABSENT."""
    fields_out = []
    present = 0
    required = 0
    ...
    if required == 0:
        # All-optional step (E4): a hit is still good news (PRESENT), but finding
...
def _steps_for_doc(
    ...
    steps = {"E1": e1_inventory(elements, doc_id)}
    for step in ("E2", "E3", "E4"):
        steps[step] = e_presence_for_step(elements, checklist, step)
    steps["E5"] = {
        "step": "E5",
        "status": "STUB",
        "note": "emit Path E one-pager TBD",
    }
    return steps


def run_path_e_for_project(project_id: str) -> dict:
    """Run Path E presence extractors for each routed student-practice doc."""
    ...
    if not isinstance(ledger, list):
        ledger = []
    checklist = load_student_practice_checklist()
    ...
    for did in sorted(doc_ids):
        ...
        row = {
            ...
            "element_count": steps["E1"].get("element_count", 0),
        }
        for step in ("E1", "E2", "E3", "E4", "E5"):
            s = steps[step]
            row[step] = {
                "status": s.get("status"),
                "note": s.get("note")
    ...
    log(f"path E → {len(doc_ids)} student_practice doc(s); E1–E4 presence extract")
    return out
